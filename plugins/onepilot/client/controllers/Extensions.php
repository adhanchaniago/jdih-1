...
class Extensions extends Controller
{
    ...
    {
        if (empty($url = Input::get('url'))) {
            throw new CmsPilotException('URL parameter is missing', 400);
        }
        ...
        if (empty($extDescribeFile)) {
            throw new CmsPilotException('Any valid Plugin or Theme vas found in the provided archive', 400);
        }
    ...
    private function installPlugin($file)
    {
        ...
        if (count($namespaceArray) != 2) {
            throw new CmsPilotException('invalid Plugin.php file', 500, null, [
        ...
        // Apply DB updates
        $updateManager = UpdateManager::instance()->resetNotes();
        ...
        if (is_null($pluginVersion) && is_null($pluginClass)) {
            throw new CmsPilotException('An error occur on the plugin install process, please retry', 500, null, [
                'plugin_file' => $file->getPath() . '/' . $file->getFilename(),
                'namespace'   => $namespace,
            ]);
        }
    ...
    private function installTheme($file)
    {
        ...
        if (empty($theme) || empty($theme['name'])) {
            throw new CmsPilotException('invalid theme.yaml file', 500, null, [
    ...
    private function download($pluginUrl)
    {
        ...
        if ($result->code != 200) {
            throw new CmsPilotException('Error when downloading plugin package', 500, null, [
    ...
    private function extract($filePath, $tmpFolder)
    {
        ...
        File::delete($filePath);
        throw new CmsPilotException('Can\'t extract file to ' . $tmpFolder, 500);
    }
